...
class FinalizacaoEventoAPITestCase(APITestCase):
    def setUp(self):
        ...
        self.url = reverse("api_v01:finalizacao", kwargs={"evento_id": self.evento.id})

    def test_atualizar_finalizacao(self):
        response_get = self.client.get(self.url)
        self.assertEqual(response_get.status_code, status.HTTP_200_OK)
        self.assertTrue(FinalizacaoEvento.objects.filter(evento=self.evento).exists())

        payload = {
            "hora_extra": True,
            "fechamento_bebidas": {"vinho": "2 garrafas"},
            "materiais_recolhidos": True,
        }

        response_patch = self.client.patch(self.url, payload, format="json")
        self.assertEqual(response_patch.status_code, status.HTTP_200_OK)
        self.assertTrue(response_patch.data["hora_extra"])
