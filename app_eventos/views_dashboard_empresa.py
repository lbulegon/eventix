...
def freelancers_empresa(request):
    ...
    from app_eventos.convite_freelancer import assinar_empresa_para_gerar_convite_publico

    empresa_param = assinar_empresa_para_gerar_convite_publico(empresa.id)
    public_path = f"{reverse('freelancer_publico:gerar_convite_publico')}?{urlencode({'empresa': empresa_param})}"
    url_gerar_convite_publico = request.build_absolute_uri(public_path)
    ...
    context = {
        ...
        'url_gerar_convite_publico': url_gerar_convite_publico,
    }
...
class NotificarFreelancersEventoView(View):
    ...
    def post(self, request, evento_id):
        """Envia notificações"""
        from app_eventos.models import Evento, Vaga
    ...
    def _enviar_notificacoes_form(self, request, evento):
        ...
        try:
            ...
            if not vagas.exists():
                ...
                return redirect('dashboard_empresa:notificar_freelancers_evento', evento_id=evento.id)
...
def notificar_freelancers_vaga_especifica(request, vaga_id):
    ...
    try:
        ...
        # Enviar notificação via Twilio real
        from app_eventos.services.notificacao_vagas import NotificacaoVagasService
        notificacao_service = NotificacaoVagasService()
        resultado = notificacao_service.notificar_nova_vaga(vaga)
...
def gerar_link_cadastro_freelancer(request):
    ...
    from app_eventos.convite_freelancer import montar_payload_convite_cadastro, parse_json_ou_post
