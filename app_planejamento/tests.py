...
class PlanejamentoInsightAPITestCase(APITestCase):
    def setUp(self):
        ...
        self.url = reverse("api_v01:planejamento_insights", kwargs={"evento_id": self.evento.id})

    def test_criar_e_listar_insights(self):
        payload = {
            "recomendacao": "Criar checklists automatizados",
            "relevancia": 5,
        }

        response = self.client.post(self.url, payload, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(InsightEvento.objects.filter(evento_base=self.evento).exists())
