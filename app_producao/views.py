...
from .serializers import CronogramaPreProducaoSerializer


class CronogramaPreProducaoView(APIView):
    ...
    def get(self, request, evento_id: int):
        ...
        queryset = CronogramaPreProducao.objects.filter(evento=evento)
        serializer = CronogramaPreProducaoSerializer(queryset, many=True)
    ...
    def post(self, request, evento_id: int):
        ...
        serializer = CronogramaPreProducaoSerializer(data=payload)
    ...
    def patch(self, request, evento_id: int):
        ...
        item = get_object_or_404(CronogramaPreProducao, evento=evento, pk=item_id)

        serializer = CronogramaPreProducaoSerializer(item, data=request.data, partial=True)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(serializer.data)
