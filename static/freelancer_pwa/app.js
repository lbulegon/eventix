...
const STATIC_BASE = CONFIG.staticBase || '/static/freelancer_pwa/';
const USER_NAME = CONFIG.freelancerName || '';
...
function Home() {
  setTitle('Eventix');
  setBack(false);
  const greeting = USER_FIRST_NAME || USER_NAME;
  return `
  <div class="grid">
    <div class="card">
      <div class="title-lg">Olá${greeting ? `, <span class="muted">${greeting}</span>` : ''}!</div>
...
function Funcoes() {
  ...
  const tabs = [
    {
      ...
      <div class="empty">
        ...
        <div class="title-lg">Nenhuma função disponível</div>
      </div>
    `,
    },
    {
      key: 'minhas',
      label: 'Minhas Funções',
      content: `
      <div class="empty">
        <div class="icon">
  ...
  const tabEls = tabs
    .map(
      (t) => `<div class="tab ${active === t.key ? 'active' : ''}" onclick="router.go('/funcoes/${t.key}')">${t.label}</div>`,
    )
    .join('');
...
function VagasParaVoce() {
  ...
  const tabEls = tabs
    .map(
      (t) => `<div class="tab ${t.key === activeKey ? 'active' : ''}" onclick="router.go('/vagas/${t.key}')">${t.label}</div>`,
    )
    .join('');
  return `
    <div class="tabs">${tabEls}</div>
...
function Perfil() {
  ...
  return `
    <div class="card">
      ...
      <div class="grid">
        <input class="input" placeholder="Nome completo" value="${USER_NAME}">
...
if ('serviceWorker' in navigator && CONFIG.swUrl) {
  window.addEventListener('load', () => {
    navigator.serviceWorker
      ...
      .catch((err) => {
        console.warn('[Eventix PWA] Falha ao registrar Service Worker', err);
      });
  });
}
